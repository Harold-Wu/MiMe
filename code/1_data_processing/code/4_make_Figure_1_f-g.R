...
meta_prob_obs_long$variable <- rep(factor(c(2,4,6,9,12)), each = 1115)
micro_prob_obs_long$variable <- rep(factor(c(2,4,6,9,12)), each = 84)
...
b1 <- ggplot(meta_prob_obs_long, aes(x=variable, y=value)) + 
...
b2 <- ggplot(micro_prob_obs_long, aes(x=variable, y=value)) + 
  geom_boxplot(outlier.shape = NA)+
