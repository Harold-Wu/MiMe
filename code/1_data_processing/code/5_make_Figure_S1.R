...
load("micro_noran_auc_test.RData")
load("meta_noran_auc_test.RData")
load("micro_auc_test.RData")
load("meta_auc_test.RData")
...
p1 <- ggplot(metas, aes(x = glm, y = glmer, color = color))+
  geom_point(size = 0.5)+
...
p2 <- ggplot(micros, aes(x = glm, y = glmer, color = color))+
  geom_point(size = 0.5)+
  scale_y_continuous(limits = c(0.485,0.95))+
  scale_x_continuous(limits = c(0.485,0.95))+
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", size = 0.3, color = "darkgray")+
