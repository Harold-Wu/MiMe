...
TASKS1[,1] <- colnames(data)[13:2434]
colnames(TASKS1) <- c("rowname", 1:repeats)
rownames(TASKS1) <- colnames(data)[13:2434]
TASK1 <- TASKS1[start:end,]

TASKS2 <- matrix(nrow = 2422, ncol = repeats+1)
TASKS2[,1] <- colnames(data)[13:2434]
colnames(TASKS2) <- c("rowname", 1:repeats)
rownames(TASKS2) <- colnames(data)[13:2434]
...
for (i in 1:nrow(TASK1)) {
  ...
  fwrite(TASK1[i,], file='../output/meta_auc_test.csv', append=TRUE)
  fwrite(TASK2[i,], file='../output/meta_noran_auc_test.csv', append=TRUE)
  print(i)
}
