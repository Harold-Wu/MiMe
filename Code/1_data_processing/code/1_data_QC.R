...
micro_data <- micro_data[, c(1:8, 8 + which(colMeans(micro_01) > 0.1))]
meta_data <- meta_data[, c(1:8, 8 + which(colMeans(meta_01) > 0.1))]
micro_data <- micro_data[which(rowSums(micro_data[,-1:-8]) != 0),]
meta_data <- meta_data[which(rowSums(meta_data[,-1:-8]) != 0),]
