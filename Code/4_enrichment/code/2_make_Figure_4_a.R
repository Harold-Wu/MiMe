...
chordDiagram(connect, group = group, col = cols, annotationTrack = c("grid"),
       order = c(unique(signi_final_top5[order(signi_final_top5$family),1]), unique(signi_final_top5[order(signi_final_top5$NPC_level3),2])),
       preAllocateTracks = list(
        track.height = mm_h(4),
        track.margin = c(mm_h(4), 0)))
highlight.sector(unique(signi_final_top5[which(signi_final_top5$family == "Lachnospiraceae"),1]), track.index = 1, col = "plum1", 
    text = "A", cex = 1.5, text.col = "black", niceFacing = TRUE)
highlight.sector(unique(signi_final_top5[which(signi_final_top5$family == "Rikenellaceae"),1]), track.index = 1, col = "plum1", 
    text = "B", cex = 1.5, text.col = "black", niceFacing = TRUE)
highlight.sector(unique(signi_final_top5[which(signi_final_top5$family == "Veillonellaceae"),1]), track.index = 1, col = "plum1", 
    text = "C", cex = 1.5, text.col = "black", niceFacing = TRUE)
highlight.sector(unique(signi_final_top5[which(signi_final_top5$family == "Prevotellaceae"),1]), track.index = 1, col = "plum1", 
    text = "D", cex = 1.5, text.col = "black", niceFacing = TRUE)
highlight.sector(unique(signi_final_top5[which(signi_final_top5$family == "Clostridiaceae"),1]), track.index = 1, col = "plum1", 
    text = "E", cex = 1.5, text.col = "black", niceFacing = TRUE)

highlight.sector(unique(signi_final_top5[which(signi_final_top5$NPC_level3 == "Cholestane steroids"),2]), track.index = 1, col = "lightblue1", 
    text = "F", cex = 1.5, text.col = "black", niceFacing = TRUE)
highlight.sector(unique(signi_final_top5[which(signi_final_top5$NPC_level3 == "Pyridine alkaloids"),2]), track.index = 1, col = "lightblue1", 
    text = "G", cex = 1.5, text.col = "black", niceFacing = TRUE)
highlight.sector(unique(signi_final_top5[which(signi_final_top5$NPC_level3 == "Prenyl quinone meroterpenoids"),2]), track.index = 1, col = "lightblue1", 
    text = "H", cex = 1.5, text.col = "black", niceFacing = TRUE)
highlight.sector(unique(signi_final_top5[which(signi_final_top5$NPC_level3 == "Open-chain polyketides"),2]), track.index = 1, col = "lightblue1", 
    text = "I", cex = 1.5, text.col = "black", niceFacing = TRUE)
highlight.sector(unique(signi_final_top5[which(signi_final_top5$NPC_level3 == "Vitamin D3 and derivatives"),2]), track.index = 1, col = "lightblue1", 
