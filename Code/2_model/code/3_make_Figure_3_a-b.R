...
f3a <- ggplot() + 
  ...
  scale_color_manual(values=c('#1E88E5', '#FFAC0A', '#999999'))+
  coord_flip()+ 
  ...
  geom_rect(aes(xmin = 305.5, xmax = 435.5, ymin = -1.02, ymax = 1.02), color = "purple", alpha = 0, linetype = "dashed")+
...
f3b <- ggplot() + 
  ...
  scale_y_continuous(limits = c(-1.05,1.05))+
  geom_hline(yintercept = 0, linetype = "dashed", color = "darkgray")+
  geom_rect(aes(xmin = 64.5, xmax = 75.5, ymin = -1.02, ymax = 1.02), color = "purple", alpha = 0, linetype = "dashed")+
  theme(plot.title = element_text(hjust = 0.5, size = 25),
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"),
        axis.title.x = element_text(size = 16), axis.title.y = element_text(size = 20),
