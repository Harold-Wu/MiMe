...
data_t0 <- data[which(data$timepoint == 2),c(-1:-12)]
...
race <- data$race
Y <- data[,-c(1:12)]
bmilk=ifelse(data[,'bmilk']=='Yes',1,0)
bmilk[is.na(bmilk)]=0
formula=ifelse(data[,'formula']=='Yes',1,0)
formula[is.na(formula)]=0
cmilk=ifelse(data[,'cmilk']=='Yes',1,0)
cmilk[is.na(cmilk)]=0
omilk=ifelse(data[,'omilk']=='Yes',1,0)
omilk[is.na(omilk)]=0

start <- 10*(jobID-1)+1
end <- ifelse(jobID==111, 1107, 10*jobID)

TASKS1 <- matrix(nrow = 1107, ncol = length(seq(1,14,0.5))+1)
TASKS1[,1] <- colnames(data)[13:1119]
colnames(TASKS1) <- c("rowname", seq(1,14,0.5))
rownames(TASKS1) <- colnames(data)[13:1119]
TASK1 <- TASKS1[start:end,]

TASKS1_2 <- matrix(nrow = 1107, ncol = length(seq(1,14,0.5))+1)
TASKS1_2[,1] <- colnames(data)[13:1119]
colnames(TASKS1_2) <- c("rowname", seq(1,14,0.5))
rownames(TASKS1_2) <- colnames(data)[13:1119]
TASK1_2 <- TASKS1_2[start:end,]

TASKS2 <- matrix(nrow = 1107, ncol = length(unique(ID))+1)
TASKS2[,1] <- colnames(data)[13:1119]
colnames(TASKS2) <- c("rowname", unique(data$subjectID))
rownames(TASKS2) <- colnames(data)[13:1119]
TASK2 <- TASKS2[start:end,]

TASKS3 <- matrix(nrow = 1107, ncol = 7)
TASKS3[,1] <- colnames(data)[13:1119]
colnames(TASKS3) <- c("rowname",
                      "Int", "Int_se", "Int_p", "age", "age_se", "age_p")
rownames(TASKS3) <- colnames(data)[13:1119]
TASK3 <- TASKS3[start:end,]

TASKS4 <- as.data.frame(matrix(nrow = 1107, ncol = 41))
TASKS4[,1] <- colnames(data)[13:1119]
...
rownames(TASKS4) <- colnames(data)[13:1119]
...
for (i in 1:nrow(TASK1)) {
  tryCatch({
    ...
    addALine(x=TASK1[i,],fname='../output/meta_pop_expected.csv')
    addALine(x=TASK1_2[i,],fname='../output/meta_pop_expected_SE.csv')
    addALine(x=TASK2[i,],fname='../output/meta_ind_expected_diff.csv')
    addALine(x=TASK3[i,],fname='../output/meta_fixef.csv')
    addALine(x=TASK4[i,],fname='../output/meta_fixef_full.csv')
