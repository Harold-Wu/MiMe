#!/usr/bin/env Rscript
#SBATCH --job-name=meta_pred
#SBATCH --time=4:00:00
#SBATCH --mem=4G
#SBATCH --array=1-243
#SBATCH --output=../logs/%x_%A_%a
#SBATCH --constraint=[intel18|intel16]
# Usage:
#
#  sbatch 1_auc_meta.R

jobID <- as.integer(Sys.getenv("SLURM_ARRAY_TASK_ID", "1"))
omic <- "meta"

library(lme4)
library(MASS)
library(dplyr)
library(pROC)
source('/mnt/research/quantgen/projects/harold/Source/utils.R')

load(paste0("/mnt/research/quantgen/projects/harold/InfantDev2/data/", omic, "_QCed_data.RData"))
data <- meta_data
#data <- data[which(data$subjectID %in% names(table(data$subjectID))[which(table(data$subjectID) >= 3)]),]

data_t0 <- data[which(data$timepoint == 2),c(-1:-8)]
tmp <- colMeans(data_t0)

ID <- data$subjectID
age <- data$age_mos
timepoint <- data$timepoint
sex <- as.factor(data$sex)
sample <- data$sampleID
race <- data$race
Y <- data[,-c(1,2,3,4,5,6,7,8)]

start <- 10*(jobID-1)+1
end <- ifelse(jobID==243, 2422, 10*jobID)

repeats <- 20

TASKS1 <- matrix(nrow = 2422, ncol = repeats+1)
TASKS1[,1] <- colnames(data)[9:2430]
colnames(TASKS1) <- c("rowname", 1:repeats)
rownames(TASKS1) <- colnames(data)[9:2430]
TASK1 <- TASKS1[start:end,]

TASKS2 <- matrix(nrow = 2422, ncol = repeats+1)
TASKS2[,1] <- colnames(data)[9:2430]
colnames(TASKS2) <- c("rowname", 1:repeats)
rownames(TASKS2) <- colnames(data)[9:2430]
TASK2 <- TASKS2[start:end,]

for (i in 1:nrow(TASK1)) {
  for (r in 1:repeats) {
    tryCatch({
    y <- Y[,start-1+i]
    full_table <- data.frame("y" = y,
                               "sample" = sample,
                               "ID" = ID,
                               "age" = age,
                               "sex" = factor(sex),
                               "race" = factor(race))
    full_table$y <- as.numeric(full_table$y != 0)
    heads <- as.data.frame(full_table %>% group_by(ID) %>% arrange(desc(age)) %>% slice_head(n=1))
    tails <- as.data.frame(full_table %>% group_by(ID) %>% arrange(desc(age)) %>% slice_tail(n=1))
    remaining <- full_table[-which(full_table$sample %in% heads$sample | full_table$sample %in% tails$sample),]
    out_table <- as.data.frame(remaining %>% group_by(ID) %>% slice_sample(n=1))
    fit_table <- full_table[-which(full_table$sample %in% out_table$sample),]
    fm1 <- glmer(y ~ age+sex+race+(age|ID), data = fit_table, family = "binomial", control = glmerControl(optimizer = "bobyqa"))
    TASK1[i,r+1] <- auc(out_table$y, predict(fm1, newdata = out_table, type = "response"))
    fm2 <- glm(y ~ age+sex+race, data = fit_table, family = "binomial")
    TASK2[i,r+1] <- auc(out_table$y, predict(fm2, newdata = out_table, type = "response"))
  }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
  }
  addALine(x=TASK1[i,],fname='../output/logistic/meta_auc_test.csv')
  addALine(x=TASK2[i,],fname='../output/logistic/meta_noran_auc_test.csv')
  print(i)
}

quit(save='no')