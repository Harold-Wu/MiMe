...
f3_1 <- ggplot() + 
  geom_jitter(data = micro_long, aes(x=Var2, y=value), position=position_jitter(0.2), cex=1, color="#0099bb") +
  geom_jitter(data = micro_long, aes(x=Var2, y=pop), position=position_jitter(0.2), cex=2, color="#FC4E07", shape=18) +
  coord_flip()+ 
  labs(title = "Microbiome", x = "ASV", y = "Expected change of proportion of zeros in the first year of life")+
...
f3_2 <- ggplot() + 
  geom_jitter(data = meta_long, aes(x=Var2, y=value), position=position_jitter(0.2), cex=1, color="#0099bb") +
  geom_jitter(data = meta_long, aes(x=Var2, y=pop), position=position_jitter(0.2), cex=1, color="#FC4E07", shape=18) +
  coord_flip()+ 
  labs(title = "Metabolome", x = "Metabolite", y = "Expected change of proportion of zeros in the first year of life")+
  scale_y_continuous(limits = c(-1.05,1.05))+
  geom_hline(yintercept = 0, linetype = "dashed", color = "darkgray")+
