...
library(lme4)
library(MASS)
source('/mnt/research/quantgen/projects/harold/Source/utils.R')

load(paste0("/mnt/research/quantgen/projects/harold/InfantDev2/data/", omic, "_filter_logit_data.RData"))
data <- meta_data
#data <- data[which(data$subjectID %in% names(table(data$subjectID))[which(table(data$subjectID) >= 3)]),]
...
start <- 10*(jobID-1)+1
end <- ifelse(jobID==112, 1115, 10*jobID)

TASKS1 <- matrix(nrow = 1115, ncol = length(seq(1,14,0.5))+1)
TASKS1[,1] <- colnames(data)[9:1123]
colnames(TASKS1) <- c("rowname", seq(1,14,0.5))
rownames(TASKS1) <- colnames(data)[9:1123]
TASK1 <- TASKS1[start:end,]

TASKS2 <- matrix(nrow = 1115, ncol = length(unique(ID))+1)
TASKS2[,1] <- colnames(data)[9:1123]
colnames(TASKS2) <- c("rowname", unique(data$subjectID))
rownames(TASKS2) <- colnames(data)[9:1123]
TASK2 <- TASKS2[start:end,]

TASKS3 <- matrix(nrow = 1115, ncol = 7)
TASKS3[,1] <- colnames(data)[9:1123]
colnames(TASKS3) <- c("rowname",
                      "Int", "Int_se", "Int_p", "age", "age_se", "age_p")
rownames(TASKS3) <- colnames(data)[9:1123]
...
for (i in 1:nrow(TASK1)) {
  tryCatch({
    ...
    addALine(x=TASK1[i,],fname='../output/logistic/meta_pop_expected.csv')
    addALine(x=TASK2[i,],fname='../output/logistic/meta_ind_expected_diff.csv')
    addALine(x=TASK3[i,],fname='../output/logistic/meta_fixef.csv')
